const BASE_URL = typeof window !== "undefined" ? window.location.origin : "http://localhost:4444";
...
async function sendToSession(sessionKey, message) {
  ...
  try {
    ...
    if (!response.ok || payload.ok === false) {
      throw new Error(
        typeof payload.error === "string" && payload.error.trim() ? payload.error : response.statusText || "Failed to send message"
      );
    }
    return payload;
  } catch (error) {
...
async function fetchModels() {
  ...
  try {
    ...
    return {
      ...
      configuredProviders: Array.isArray(payload.configuredProviders) ? payload.configuredProviders : []
    };
  } catch (error) {
