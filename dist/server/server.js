...
const GLOBAL_STORAGE_KEY = /* @__PURE__ */ Symbol.for(
...
const GLOBAL_EVENT_STORAGE_KEY = /* @__PURE__ */ Symbol.for(
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } =
    await import('./assets/_tanstack-start-manifest_v-DSLM-Es9.js')
    ...
    try {
      let serializeResult = function (res2) {
        ...
        if (res2 !== void 0) {
          ...
          const rawStreamPlugin = createRawStreamRPCPlugin((id, stream2) => {
          ...
          if (typeof serializedContext === 'string') {
            try {
              ...
              if (
                ...
                params.context = safeObjectMerge(context, deserializedContext)
              }
            } catch (e) {
              if (false);
            }
...
async function loadEntries() {
  const routerEntry = await import('./assets/router-Bxwn-W7k.js').then(
    (n) => n.V,
  )
  ...
  const startRequestResolver = async (request, requestOpts) => {
    ...
    try {
      ...
      if (SERVER_FN_BASE && url.pathname.startsWith(SERVER_FN_BASE)) {
        ...
        const ctx2 = await executeMiddleware(
          [...middlewares2, serverFnHandler],
          {
            request,
